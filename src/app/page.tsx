...
export default function Home() {
  ...
  const handleExportPdf = () => {
    ...
    try {
        ...
        pdf.setFontSize(14);
        pdf.text('Generated by InsightForge', pdf.internal.pageSize.getWidth() / 2, pageHeight / 2 + 10, { align: 'center' });
        
        pdf.setFontSize(12);
        const date = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
        pdf.text(date, pdf.internal.pageSize.getWidth() / 2, pageHeight / 2 + 20, { align: 'center' });
        addPageHeaderAndFooter();
        ...
        sectionOrder.forEach(sectionKey => {
            if (report[sectionKey]) {
                ...
                pdf.line(pageMargin, y, pageMargin + contentWidth, y);
                y += 10;
                
                // Section Content
                pdf.setFont('times', 'normal');
                pdf.setFontSize(12);
                const contentLines = pdf.splitTextToSize(report[sectionKey], contentWidth);
                const lineHeight = 7;
